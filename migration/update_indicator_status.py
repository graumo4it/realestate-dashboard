...
SCHEDULE = [
    # ── Ежемесячные ────────────────────────────────────────────────────────────
    {   # CBR primary: 6.1–6.27 (ЦБ публикует 02_02/02_03 ~3–5-го числа)
        "codes": [
...
def get_expected_period(sched: dict, today: date) -> date | None:
    ...
    if periodicity == "monthly":
        # Данные публикуются раз в месяц с лагом 1 мес.
        # Если сегодня ≥ run_day → ожидаем данные за прошлый месяц
        # Если сегодня < run_day → парсер ещё не запускался в этом месяце,
        #                           ожидаем данные за позапрошлый месяц
        if today.day >= run_day:
            return _prev_month_start(today)
        else:

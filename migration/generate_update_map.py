...
PARSERS = {
    ...
    "rosstat_income": {
        ...
        "notes": "Скачивает актуальный urov_10kv_*kv-*.xlsx напрямую с Росстата. "
                 "Парсит квартальные строки для 1.2. SSL-сертификат Росстата — verify=False.",
        "codes": ["1.2"],
    },
    "domrf": {
...
CALC_MANUAL = [
    {
        "script": "load_rosstat_annual.py",
        "codes": ["1.2.y"],
        "formula": "Строка «Год» из urov_10kv_Nkv-YYYY.xlsx (rosstat.gov.ru/folder/13397)",
        "inputs": "urov_10kv_Nkv-YYYY.xlsx — тот же файл, что и для 1.2",
        "notes": "Companion-индикатор для chart.html (режим «Год» на квартальном графике 1.2). "
                 "Обновляется вручную раз в год когда нужно добавить новое годовое значение.",
    },
]

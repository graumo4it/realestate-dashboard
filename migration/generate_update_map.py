...
PARSERS = {
    "domrf_web": {
        ...
        "schedule": "5-е число, 08:00 UTC (job: day5_domrf_web, retry +5д)",
    ...
    "rosreestr": {
        ...
        "schedule": "1 фев/май/авг/ноя, 08:00 UTC (job: quarterly_rosstat)",
        "status": STATUS_AUTO_PARSER_OK,
        "notes": "Готов, тестировался. Скачивает кумулятивные XLS, считает квартальные значения.",
        "codes": ["5.1"],
    },
    "cbr_primary": {
        "name": "cbr.py (group='primary')",
        "file": "parsers/cbr.py",
        "source": "Банк России (cbr.ru)",
        "url": "cbr.ru (02_02_Mortgage.xlsx, 02_03_Scpa_mortgage.xlsx)",
        "schedule": "1-е число, 08:00 UTC (job: day1_cbr_primary)",
        "status": STATUS_AUTO_PARSER_OK,
        "notes": "Ежемесячная ипотека CBR (6.1–6.27). CBRParser(group='primary').",
        "codes": [
    ...
    "cbr_ihc": {
        "name": "cbr.py (group='ihc')",
        "file": "parsers/cbr.py",
        "source": "Банк России + ДОМ.РФ (cbr.ru + наш.дом.рф)",
        "url": "cbr.ru (02_41_Mortgage_ihc.xlsx) + ДОМ.РФ API субсидии",
        "schedule": "7-е число, 08:00 UTC (job: day7_cbr_ihc)",
        "status": STATUS_AUTO_PARSER_OK,
        "notes": "ИЖС и субсидии CBR (6.36–6.87). CBRParser(group='ihc'). Объединяет бывший SubsidyOnlyParser.",
        "codes": [
            # ИЖС from 02_41
            "6.70","6.71","6.72","6.73","6.76","6.77","6.78","6.79",
            "6.82","6.83","6.84","6.85",
            "6.74","6.75","6.80","6.81","6.86","6.87",
            # субсидии from ДОМ.РФ API
            "6.36","6.37","6.38","6.39","6.40","6.41","6.42","6.43","6.44","6.45",
        ],
    },
    "rosstat_monthly": {
        "name": "rosstat.py (monthly)",
        "file": "parsers/rosstat.py + migration/fetch_fedstat.py",
        "source": "fedstat.ru (ЕМИСС / Росстат)",
        "url": "fedstat.ru (EMISS API)",
        "schedule": "20-е число, 11:00 UTC (job: monthly_rosstat, retry +5д)",
        "status": STATUS_AUTO_PARSER_OK,
        "notes": "Ежемесячные данные: 2.1/2.2/2.3. ⚠️ fedstat блокирует облачные IP — только локальный запуск.",
        "codes": ["2.1","2.2","2.3"],
    },
    "rosstat_quarterly": {
        "name": "rosstat.py (quarterly)",
        "file": "parsers/rosstat.py + migration/fetch_fedstat.py",
        "source": "fedstat.ru (ЕМИСС / Росстат)",
        "url": "fedstat.ru (EMISS API)",
        "schedule": "1 фев/май/авг/ноя, 08:00 UTC (job: quarterly_rosstat)",
        "status": STATUS_AUTO_PARSER_OK,
        "notes": "Квартальные данные: 1.3, 4.4, 4.5. ⚠️ fedstat блокирует облачные IP — только локальный запуск.",
        "codes": [
            "1.3",
    ...
    "rosstat_annual": {
        "name": "rosstat.py (annual housing stats)",
        "file": "parsers/rosstat.py + migration/fetch_fedstat.py",
        "source": "fedstat.ru (ЕМИСС / Росстат)",
        "url": "fedstat.ru (EMISS API)",
        "schedule": "5 июня, 08:00 UTC (job: annual_housing_stats, retry +10д)",
        "status": STATUS_AUTO_PARSER_OK,
        "notes": "Ежегодные данные: 2.9, 2.11, 2.12, 2.13. ⚠️ fedstat блокирует облачные IP — только локальный запуск. 2.13.ext — только до 2015, не обновляется.",
        "codes": ["2.9","2.11","2.12","2.13","2.13.ext"],
    },
    "rosstat_income": {
...
CALC_SCHEDULED = [
    {
        ...
        "trigger": "annual_population_dev → ежегодно 15 марта; обновляется при добавлении новых значений в скрипт",
    },
    {
        "script": "calc_housing_per_capita.py",
        "schedule": "20-е, в составе monthly_rosstat (retry +5д)",
        "codes": ["2.6", "2.7", "2.8"],
        "formula": "2.6=2.1/1.1; 2.7=2.2/1.1; 2.8=(2.1−2.2)/1.1",
        "inputs": "2.1 (ввод всего, тыс. кв. м), 2.2 (ввод ИЖС), 1.1 (население, тыс. чел.)",
        "trigger": "monthly_rosstat → 2.1/2.2 обновились; 1.1 актуален (annual_population_dev ранее)",
    },
    {
        "script": "calc_avg_apt_area.py",
        "schedule": "5-е, в составе day5_domrf_web (retry +5д)",
        "codes": ["3.5"],
        "formula": "3.5 = 3.3 / 3.4",
        "inputs": "3.3 (жилая площадь), 3.4 (кол-во квартир)",
        "trigger": "day5_domrf_web → 3.3/3.4 обновились",
    },
    {
    ...
    },
    {
        ...
        "trigger": "annual_population_dev → migrate_population + 5.1 актуальны",
    },
    {
    ...
    },
    {
        ...
        "schedule": "1 фев (annual_companion, retry 1-е кажд. мес. до появления данных)",
        "codes": ["1.3.y"],
        "formula": "1.3.y[Y] = (Q1 + Q2 + Q3 + Q4) / 4 из данных 1.3",
        "inputs": "1.3 (квартальная зарплата, обновляется quarterly_rosstat)",
        "trigger": "annual_companion → ежегодно 1 февраля + retry до появления 1.3 за все 4 квартала",
    },
    {
        "script": "calc_housing_provision.py",
        "schedule": "5 июн (annual_housing_stats, retry +10д)",
        "codes": ["2.10"],
        ...
        "trigger": "annual_housing_stats → rosstat.py (2.9) завершился успешно",
    },
    {
        "script": "calc_housing_need.py",
        "schedule": "5 июн (annual_housing_stats, после calc_housing_provision)",
        ...
        "trigger": "annual_housing_stats → calc_housing_provision → 2.10 обновился",
    },
]

...
# Формат 1: зарплата (57823) — накопительные периоды ("январь-март", "январь-июнь" и т.д.)
# Примечание: 1.2 (доходы, 57039) перенесён в fetch_income_rosstat.py
CUMULATIVE_QUARTERS = {
    ...
    # Формат порядковый (резерв для других индикаторов)
    "i квартал":       (1,  "Q1"),
    "ii квартал":      (4,  "Q2"),
    "iii квартал":     (7,  "Q3"),
    "iv квартал":      (10, "Q4"),
...
INDICATORS = [
    ...
    *[
    ...
    # ── 1.2 УДАЛЁН из fedstat ─────────────────────────────────────────────────
    # Источник перенесён: migration/fetch_income_rosstat.py
    # Скачивает urov_10kv_Nkv-YYYY.xlsx с rosstat.gov.ru/folder/13397
    # Запускается автоматически в scheduler.py (day10, после rosstat.py)
    # ──────────────────────────────────────────────────────────────────────────
    # fedstat_id 57039 оставлен здесь для справки:
    # quarter_ids: 1540222=Q1, 1540224=Q2, 1540226=Q3, 1540227=Q4
    # payload_base содержал длинный список selectedFilterIds=57831_168xxxx (субъекты РФ)
    # ─────────────────────────────────────────────────────────────────────────
]

_REMOVED_INDICATOR_12 = {  # архив — не используется
        "fedstat_id": "57039",
        "code":       "1.2",

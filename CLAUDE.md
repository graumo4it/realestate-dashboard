...
- `period_type`: четыре значения — `period` (за период: потоки — выдачи, ввод жилья), `period_start` (на начало отчётного периода: UC ДОМ.РФ, ипотечный долг 6.19–6.27), `period_end` (на конец отчётного периода: жилфонд, коэффициенты поглощения/запуска), `on_date` (на дату: квартирография, запасы стройки). Миграция `010_period_type_expansion.sql`; вспомогательная функция во frontend — `periodTypeLabel(type)` в `utils.js`
...
**Недавно завершено:**
- ✅ **Метастрока: источник + тип данных** (`migration/010_period_type_expansion.sql`, `frontend/js/utils.js`, `frontend/chart.html`, `frontend/js/chart-page.js`, `frontend/js/combo-page.js`, `frontend/js/subsidy-purpose-page.js`, все 46 комбо-HTML): `period_type` расширен до 4 значений (`period`, `period_start`, `period_end`, `on_date`); все 48 бывших `point_in_time` индикаторов реклассифицированы. Метастрока на всех страницах теперь показывает источник + тип данных (вместо даты публикации + источника + единицы). Функция `periodTypeLabel(type)` добавлена в `utils.js`.
...
- ✅ **Источник `mortgage_subsidy`** (БД): 128 индикаторов `6.36–6.58.x` переведены с `source=cbr` → `source=domrf`; ДОМ.РФ является первичным публикатором статистики по субсидированным программам ипотеки.

...
> Статус на 26 мая 2026: основные шаги плана реализованы. Файл оставлен как справочник по исходным проблемам, источникам и принятым техническим решениям. Актуальное расписание см. в `parsers/scheduler.py`, актуальный агентский контекст — в `AGENTS.md`.

## Контекст

Исходная диагностика показала, что старые три парсера в `parsers/` фактически не использовались и были сломаны:
...
**Цель плана**: создать рабочую систему, которая:
...
Оставшаяся работа: прогнать dry-run / ручные запуски на локальной БД после загрузки нужных файлов и зафиксировать результат в `indicator_update_map.xlsx`.
...
| Файл | Действие |
|------|----------|
| `parsers/base.py` | ✅ `get_last_dates()`, инкрементальный `run()`, `DO NOTHING`, `upsert_update_to_db()` |
| `parsers/cbr.py` | ✅ Переработан; поддерживает `--group primary` и `--group ihc` |
| `parsers/rosstat.py` | ✅ Делегирующая обёртка над `migration/fetch_fedstat.py` |
| `parsers/domrf.py` | ✅ Оркестратор `migration/migrate_*.py` и расчётных скриптов |
| `parsers/scheduler.py` | ✅ Новое расписание с retry |
| `migration/fetch_fedstat.py` | ✅ Инкрементальный фильтр + новые INDICATOR-записи |
| `parsers/requirements.txt` | ✅ Зависимости зафиксированы |
